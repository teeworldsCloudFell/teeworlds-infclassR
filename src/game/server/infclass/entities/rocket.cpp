...
CRocket::CRocket(CGameContext *pGameContext, int Owner, vec2 Pos, vec2 Dir)
	: CInfCEntity(pGameContext, CGameWorld::ENTTYPE_PROJECTILE, Pos, Owner, 3)
{
	m_Direction = Dir;
...
void CRocket::ProcessExplosion()
{
	...
	for(int i = 0; i < Num; i++)
	{
		...
		float ForceValue = 2 * l;
		if(pCharacter->IsHuman())
		{
			Dmg = 0;
		}

		apEnts[i]->TakeDamage(ForceDir * ForceValue, Dmg, GetOwner(), WEAPON_GUN, TAKEDAMAGEMODE_NOINFECTION);
	}
